...
void setup() {
  ...
  // hits_needed=1: fire on the very first near reading each revolution.
  // hits_needed=2 would require 2 CONSECUTIVE LIDAR samples on the block.  At high RPM
  // the block is visible for < one 4 ms LIDAR frame, so the zero would fire only every
  // 2-4 revolutions → omega_period_dps would be 2-4× too slow → text spread across
  // multiple revolutions.  With a clear environment, single-hit sensitivity is fine.
  zref.hits_needed = 1;
  // fars_needed=1: re-arm after just one far reading.  Still requires a full
  // far-zone pass before the state machine can fire again, preventing double-fires.
  zref.fars_needed = 1;
...
void loop() {
  ...
  jyro_read_xyz_g(ax_g, ay_g, az_g);
  ...
  float wobble_dps = sqrtf(gx * gx + gy * gy);

  float tilt_deg = 0.0f;
  float accel_norm = sqrtf(ax_g * ax_g + ay_g * ay_g + az_g * az_g);
  if (accel_norm > 0.0001f && fabsf(accel_norm - 1.0f) <= ACCEL_NORM_TOL_G) {
    float cos_tilt = az_g / accel_norm;
  ...
  if (tilt_deg > TILT_MAX_DEG) {
    last_tilt_bad_ms = now_ms;
  }
  bool tilt_stable = (now_ms - last_tilt_bad_ms) >= TILT_STABLE_MS;

  // Wobble threshold scales with spin so a fast but stably-tilted top isn't blocked.
  // A top at spin_dps with small tilt produces gx/gy ≈ spin_dps * sin(tilt).
  // WOBBLE_SPIN_RATIO of 0.30 tolerates up to ~17° of static tilt at any speed.
  float wobble_threshold = spin_dps_abs * WOBBLE_SPIN_RATIO;
  if (wobble_threshold < WOBBLE_BASE_DPS) wobble_threshold = WOBBLE_BASE_DPS;

  if (wobble_dps > wobble_threshold) {
    last_wobble_bad_ms = now_ms;
  }
  bool wobble_stable = (now_ms - last_wobble_bad_ms) >= WOBBLE_STABLE_MS;
  bool zero_stable = tilt_stable && wobble_stable;
  ...
  if (luna_read_mm_latest(d_mm)) {
    ...
    // Period-plausibility gate: once the period is established, only accept a zero
    // that arrives near an integer multiple of the expected period (±ZERO_PERIOD_TOL).
    // This stops stray near-reads from corrupting the phase at the wrong angle.
    // When the system is stale (no zero for ZERO_STALE_MS) the gate opens so the
    // system can re-sync from scratch.
    bool zero_in_window = true;
    if (have_period && last_zero_us != 0) {
      bool is_stale = (now_ms - (uint32_t)(last_zero_us / 1000u)) > ZERO_STALE_MS;
      if (!is_stale) {
        float elapsed_us   = (float)(uint32_t)(micros() - last_zero_us);
        float period_us    = 360.0f * 1e6f / omega_period_dps;
        if (period_us > 0.0f) {
          float frac = fmodf(elapsed_us / period_us, 1.0f);
          if (frac < 0.0f) frac += 1.0f;
          float err  = (frac < 0.5f) ? frac : (1.0f - frac);
          zero_in_window = (err < ZERO_PERIOD_TOL);
        }
      }
    }
    // Always run the near/far state machine so `armed`/`far_hits` stay in
    // sync with the physical block passes -- otherwise a wobble/tilt blip
    // during the far-zone pass leaves `armed=false` and the *next* real
    // zero-crossing gets silently swallowed (whole revolution skipped).
    // The zero_stable/zero_in_window gates only decide whether we trust a
    // detected crossing enough to resync the phase/period from it.
    bool zero_detected = luna_update_zero(zref, d_mm, theta_deg);
    if (zero_detected && zero_stable && zero_in_window) {
      ...
      debug_print_ble("ZERO SET (block detected)\n");
    }
    ...
    if (pov_enabled) {
      ...
      if (pov_ready) {
        float dt_s = (micros() - last_zero_us) * 1e-6f;
        if (dt_s >= 0.0f && dt_s < 10.0f) {
          // theta_deg integrates live gyro (blended with the last revolution's
          // average rate) and is reset to 0 at each zero-crossing, so it tracks
          // this revolution's actual progress even while the top is decelerating
          // -- unlike a pure dt_s * omega_period_dps extrapolation from the
          // previous (faster) revolution, which runs ahead and causes jitter.
          heading = fmodf(fabsf(theta_deg), 360.0f);
          heading_valid = true;
        }
      }
      if (heading_valid) {
